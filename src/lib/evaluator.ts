import { TermKind, type Expr } from './types';

export function normalise(expr: Expr, steps: number = 10): Expr {
	if (steps === 0) {
		return expr;
	}
	...
	return normalise(next, steps - 1);
}

function normal_step(expr: Expr): Expr | undefined {
	const top = beta(expr);
	if (top) {
		return top;
	} else {
